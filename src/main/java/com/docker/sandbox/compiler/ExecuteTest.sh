...
TEST_CASE_FILE_PATH=$SUBMISSION_FOLDER_PATH"/input-"$TEST_CASE_NO".txt"
OUTPUT_FILE_PATH=$SUBMISSION_FOLDER_PATH"/output-"$TEST_CASE_NO".txt"
USER_OUTPUT_FILE_PATH=$SUBMISSION_FOLDER_PATH"/user-output-"$TEST_CASE_NO".txt"

if [ "$EXECUTABLE" = "" ]; then
  docker exec $CONTAINER_ID bash -c "ulimit -v $((MEMORY_LIMIT_IN_MB*1024));timeout $TIME_LIMIT $COMPILER $SUBMISSION_FOLDER_PATH/$FILE_NAME < $TEST_CASE_FILE_PATH > $USER_OUTPUT_FILE_PATH"
else
